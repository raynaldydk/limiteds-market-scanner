...
function render() {
  ...
  const accountAssets=accounts.reduce((sum,a)=>sum+accountAssetValue(a),0);
  const spending=purchases.reduce((sum,p)=>sum+businessCost(p),0), revenue=sales.reduce((sum,s)=>sum+integer(s.price),0);
  ...
  element('kpiRap').textContent=number(totalRap);element('kpiEstimatedRobux').textContent=number(estimatedRobux);element('kpiPortfolioIdr').textContent=idr(estimatedRobux*rate+accountAssets);
