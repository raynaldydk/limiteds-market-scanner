...
The Executive Summary combines Account Manager, Buying, Sell Robux, and portfolio snapshot data. It reports account count and Plus status, Limiteds RAP, estimated Robux, portfolio IDR, purchase spending, sales revenue, net cash flow, liquid and pending Robux, account-level portfolio contribution with a Grand Total row, purchase mix, and the latest ten activities. Its period control supports All Time, a specific date, a calendar month, or a calendar year. The selected period filters spending, sales, cash flow, purchase mix, and activity. Historical modes use the period's latest snapshot as closing portfolio/account state and show opening, average, and change; All Time uses current Account Manager values. Account Manager replaces the automatic snapshot for the current Jakarta calendar day after a successful inventory refresh, while **Save Snapshot** retains additional manual snapshots. Snapshots persist in Git-ignored `data/account-snapshots.json`. `Estimated Robux = ROUND(70% x RAP) + Robux + Pending` per account. Parent accounts carry an asset value of Rp15,000. Other accounts with a send limit of at least 10,000 carry Rp25,000; lower-limit accounts, including 1,000, carry Rp0. `Portfolio IDR = Estimated Robux x selected rate + Account Asset IDR`, and `Net Cash Flow = Robux sales revenue - business purchase spending`. When a completed purchase contains `businessCostIdr`, personal-use cost is excluded from Total Spending, Purchase Mix, Recent Activity, and Net Cash Flow while the original purchase price remains preserved in the record.
...
The Account Manager adds accounts by exact Roblox username. It automatically retrieves the public user ID, display name, profile link, avatar, public collectible inventory, and total collectible RAP. Roblox communities can also be added by name with an optional community ID and manually maintained group Robux and pending Robux balances. Community funds are included in portfolio totals and snapshots but are skipped during public account inventory refreshes. Rows can be dragged into a custom order, which is persisted with the account records. The saved account record also includes editable Robux, Robux Pending, Robux Send Limit, Robux Send Limit Used, Robux Plus status and subscription date, Underage, and Parent fields. Editing the Plus subscription date recalculates its 30-day expiration. Underage defaults to green `False`; `sssssssel6` is marked red `True`. Parent defaults to `False`. Records are written to `data/accounts.json`, with browser storage retained as a migration and recovery backup. It requires no Roblox password, OAuth app, or `.ROBLOSECURITY` cookie. Private inventories and community revenue cannot be retrieved.
...
The Account Manager includes a persistent Robux Sell Rate selector with 130, 135, and 140 IDR options. Estimated IDR is `Estimated Robux x selected Robux Sell Rate + Account Asset IDR`. A Parent account is valued at Rp15,000. Otherwise, an account with a send limit of at least 10,000 is valued at Rp25,000 and a lower-limit account is valued at Rp0.
...
The Buying page tracks Limiteds, Roblox Plus, Robux, accounts, and other purchases. Roblox Plus entries require only an Account Manager account, IDR price, and date; saving one activates Plus for 30 days. Robux entries require an account, Robux amount, IDR price, and date; saving one adds that amount to the selected Account Manager balance. Account purchases require a new Roblox username, price, and date; the username is verified through Roblox and added to Account Manager with public profile and inventory data. Roblox Plus, Robux, Account, and Other purchases are treated as expenses with `Profit Est. = -Purchase Cost`. Limited purchases can be paid in IDR or Robux; Robux payment requires an account, deducts the Robux cost from its balance, and uses `Robux Cost x Rate` as the IDR cost basis. Other records store an optional owner, description, price, and date. Limited purchases additionally support exact-name RAP scanning, Total RAP and Estimated Robux KPIs, and the formulas `70% RAP = ROUND(0.7 x RAP)`, `Est. Revenue = 70% RAP x Rate`, `Minimum Robux Sell = CEILING(Purchase Price / Rate)`, and `Profit Est. = Est. Revenue - Purchase Price`. Existing Limited records remain compatible. Purchases persist locally in the Git-ignored `data/limited-purchases.json` file.
