...
The Report Statement at `/report-statement.html` reads Accounts, Buying, Sell Robux, and snapshot APIs in parallel. It filters cash transactions by an inclusive local-date range, groups business spending by purchase type, and reports sales revenue less spending as net cash flow. Its ending portfolio position uses the latest snapshot on or before the selected end date, falling back to current accounts when no suitable snapshot exists. The report includes a transaction schedule and print-specific styling.
